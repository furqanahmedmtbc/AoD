...
def get_img(mdata_path):
    ...
    img_size = 64
    #img = io.imread(mdata_path)
    pix = Image.open(mdata_path)
    img = np.array(pix)
    print('Image size was:' + str(img.shape))
    img = imresize(img, (img_size, img_size, 3))
    return img

def get_dataset(dataset_path='Data/Training/'):
    ...
    try:
        ...
        for label in labels:
            ...
            for data in listdir(datas_path):
                ...
                count_data += 1
                print('Processing image #'+ str(count_data) + ' from label: ' + str(label))
        # Create dateset:
        import keras
